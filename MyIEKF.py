...
class MyIEKF:
    ...
    def set_intial_conditions(self, x_0, Ex_0, P_0):
        self._x_0 = x_0
        self._Ex_0 = Ex_0
        self._P_0 = P_0
    ...
    def run_filter(self, Z_k, U_k, dt, iterate=True):
        ...
        self._N = Z_k.shape[0]
        self._dt = dt
        self._nm = Z_k.shape[1]  # number of measurements
        self._m = U_k.shape[1]  # number of inputs
        ...
        for k in range(self._N):
            u_vec = U_k[k, :].reshape((self._m, 1))
            z_vec = Z_k[k, :].reshape((self._nm, 1))

            # Prediction x(k+1|k)
            x_kk_1 = self._integration_routine(self._calc_f, x_k_1k_1, u_vec, [ti, tf])
            x_kk = x_kk_1
            # Predicted output z(k+1|k)
            z_kk_1 = self._calc_h(x_kk_1, u_vec)

            # Calculate Phi(k+1,k) and Gamma(k+1,k)
            Fx = self._calc_Fx(None, u_vec)
            ...
            if iterate:
                # FIXME Does not converge, not sure initial conditions are the problem
                eta2 = x_kk_1
                epsilon = 1e-10
                err = 2 * epsilon
                max_iters = 100
                for i in range(max_iters):
                    ...
                    # Observation state
                    z_p = self._calc_h(eta1, u_vec)

                    eta2 = x_kk_1 + K_gain @ (z_vec - z_p - Hx @ (x_kk_1 - eta1))
                ...
                x_k_1k_1 = x_kk_1 + K_gain @ (z_vec - z_kk_1)
            ...
            store_x.append(x_k_1k_1)
            store_stdx.append(stdx_cor)
